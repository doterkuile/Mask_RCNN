...
class MaskRCNNNode(object):
    ...
    def run(self):
        ...
        image_sub = rospy.Subscriber(self._rgb_input_topic, Image,
                         self._image_callback, queue_size=1)

        rate = rospy.Rate(self._publish_rate)
        while not rospy.is_shutdown():

            if not (self.keepNodeActive(image_sub.get_num_connections())):
                rospy.signal_shutdown("Rosbag was finished")
    ...
    def keepNodeActive(self, num_publishers):

        elapsed_time = rospy.get_rostime() - self._last_update

        bag_active = num_publishers > 0
        callback_inactive = (float(elapsed_time.to_sec()) > 5.0)
        if ((not bag_active) and self._callback_started and callback_inactive):
            return False
        return True
